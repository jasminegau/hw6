...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
    ...
    if(total/CAPACITIES[mIndex_] >= resizeAlpha){
      this->resize();
    }
    if (internalFind(p.first) != nullptr){
      internalFind(p.first)->item.second = p.second;
    }
    else {
      HASH_INDEX_T t = probe(p.first);
      if (t == npos){
